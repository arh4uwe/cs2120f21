...
/-
We have from our hypothesis that w = z. By the substitutivity 
of equals, w = z can be rewritten to w = w, using the fact 
that w = z. From here, we can prove w = w by the reflexive 
property of equality. QED.
-/
...
def prop_1 : Prop := 
  ∀ {K : Type} (x y z w : K), (x = y) → (y = z) → (w = z) → (z = w)
...
/-
To prove (∀ x, P x), we first assume that x exists. Then, we 
...
the and introduction rule to a proof of (P ∧ Q) 
and a proof of R. What remains, then, is to obtain 
these proofs. But this is easily done by the 
application of the and elimination rule to p_qr. 
QED. 
-/
